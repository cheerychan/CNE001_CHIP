...
void ts_init(void)
{
	...
	writel(0x0060000a, 0x6c02c);
	writel(0x00000009, 0x6c030);
	writel(0x00000008, 0x6c034);
	writel(0x00000007, 0x6c038);
...
void reduce_boost_while(void){
	unsigned int power_cpu;
	if (p_int->PCU_IRQ_PREF_REDUCE.reg.pcu_irq_pref_reduce){
        power_cpu=p_int->PMP_GPU_PERF_REQ.uint;
		PMP_print_f("gfx request reduce power budget is ");
	    PMP_print_f(tohex_str(power_cpu));
	    PMP_print_f("\n");
	}
	else if(p_int->PCU_IRQ_PREF_BOOST.reg.pcu_irq_pref_boost){
        power_cpu=p_int->PMP_GPU_PERF_REQ.uint;
		PMP_print_f("gfx request boost power budget is ");
	    PMP_print_f(tohex_str(power_cpu));
	    PMP_print_f("\n");
